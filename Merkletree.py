...
bc::hash_digest create_merkle(bc::hash_digest_list& merkle)
{
    ...
    while (merkle.size() > 1)
    {
        // If number of hashes is odd, duplicate last hash in the list.append
        if (merkle.size() % 2 != 0)
        ...
        for (auto it = merkle.begin(); it != merkle.end(); it += 2)
        {
            ...
            bc::data_chunk contact_data(bc::hash_size * 2);
            auto contact = bc::make_serializer(contact_data.begin());
            concat.write_hash(*it);
            concat.write_hash(*(it + 1));
            assert(contact.iterator() == contact_data.end());
...
int main()
{
    ...
    std::count << "Result: " << bc::encode_hex(merkle_root) << std::endl;
    return 0;
}