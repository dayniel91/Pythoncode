# Selects outputs from a UTXO list using a greedy algorthim.

from sys import argv

class OutputInfo:
     def__init__(self, tx_hash, tx_index, value):
...
# Return outputs list and remaining change to be sent to
# a change address.
def selct_outputs_greedy(unspent, min_value):
    # Fail if empty.
    if not unspent:
        return None
    ...
    key_func = lamba utxo: utxo.value
    if greaters:
    ...
    # Rearrange them from biggest to smallest. We want use the least
    # amount of inouts as possible.
